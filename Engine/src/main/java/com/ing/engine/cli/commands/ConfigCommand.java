            ...
            try {
                if (global) {
                    ...
                    if (globalConfig.exists()) {
                        ...
                        System.out.println(
                            cli.getOutputFormatter().formatKeyValue(propsToMap(props))
                        );
                    } else {
    ...
    public static class GetCommand implements Callable<Integer> {
        ...
        public Integer call() {
            ...
            try {
                ...
                if (global) {
                    configFile =
                        new File(System.getProperty("user.home"), ".ingenious/config.properties");
                } else {
                    String path = projectPath != null ? projectPath : cli.getProjectPath();
                    if (path == null || path.isEmpty()) {
    ...
    public static class SetCommand implements Callable<Integer> {
        ...
        public Integer call() {
            ...
            try {
                ...
                Properties props = configFile.exists()
            ...
            if (check) {
                ...
                rows.add(
                    Arrays.asList(
                        "Chrome",
                        "chromedriver",
                        getDriverVersion("chromedriver"),
                        checkDriverStatus("chromedriver")
                    )
                );
                rows.add(
                    Arrays.asList(
                        "Firefox",
                        "geckodriver",
                        getDriverVersion("geckodriver"),
                        checkDriverStatus("geckodriver")
                    )
                );
                rows.add(
                    Arrays.asList(
                        "Edge",
                        "msedgedriver",
                        getDriverVersion("msedgedriver"),
                        checkDriverStatus("msedgedriver")
                    )
                );
                rows.add(
                    Arrays.asList(
                        "Safari",
                        "safaridriver",
                        getDriverVersion("safaridriver"),
                        checkDriverStatus("safaridriver")
                    )
                );
